...
class MolTestDataset(Dataset):
    def __init__(self, tdc, datasetType):
        super(Dataset, self).__init__()
        df = get_data_df(tdc, yLabel="Y", datasetType=datasetType)
        self.smiles_data, self.labels = df["Drug"].values, df["Y"].values
...
class MolTestDatasetWrapper(object):
    ...
    def create_data_loader(self, dataset_type):
        dataset = MolTestDataset(tdc=self.tdc, datasetType=dataset_type)
        loader = DataLoader(
            dataset,
